  ...
  async function handleSignin(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 404 && data.error === 'no_account') {
        // No account — switch to signup mode with email pre-filled
        setMode('join');
        setStatus('error');
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        setMode('signin');
        setStatus('error');
        setMessage('You already have an account. Enter your email to get a sign-in link.');
        return;
      }
      if (!res.ok) throw new Error(data.error);
      setStatus('success');
      ...
      <section style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px', background: 'var(--bg-white)' }}>
        <div style={{ maxWidth: 480, textAlign: 'center' as const }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--ink-2)', lineHeight: 1.72, marginBottom: 24 }}>
            We sent a sign-in link to <strong>{successEmail}</strong>. Click it to access your member dashboard — no password needed.
          </p>
          <p style={{ fontFamily: "'Fira Code', monospace", fontSize: 10, letterSpacing: '0.1em', color: 'var(--ink-4)' }}>
            Didn&apos;t get it? Check your spam folder or{' '}
            <button onClick={reset} style={{ background: 'none', border: 'none', color: 'var(--g-700)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'inherit', padding: 0, letterSpacing: 'inherit' }}>
              try again
            </button>.
          </p>
        </div>
      ...
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        {mode === 'signin' ? (
          <>
            ...
            <p style={{ fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.72, marginBottom: 32 }}>Enter your email and we&apos;ll send you a magic link to access your dashboard instantly.</p>
